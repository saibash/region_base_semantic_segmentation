...
class ConfusionMatrix:
  ...
  def count_predicted_batch(self, ground_truth_vec, predicted, number_of_added_elements=1):  # added
      for i in range(ground_truth_vec.shape[0]):
          self.confusion_matrix[ground_truth_vec[i], predicted[i]] += number_of_added_elements
  ...
  def get_precision(self):
    precision = np.zeros(self.number_of_labels, dtype=np.float32)
    recall = np.zeros(self.number_of_labels, dtype=np.float32)
    f1_score = np.zeros(self.number_of_labels, dtype=np.float32)
    for classes in range(self.number_of_labels):
        tp = self.confusion_matrix[classes][classes]
        fp = 0
        for i in range(self.number_of_labels):
            fp += self.confusion_matrix[i, classes]
        fp = fp - tp
        fn = sum(self.confusion_matrix[:][classes])-self.confusion_matrix[classes][classes]
        # print ("tp : %.3f, fp: %.3f, fn : %.3f" %(tp,fp,fn))
        precision[classes] = tp / (tp + fp + +.0000000001)
        recall[classes] = tp / (tp + fn + .0000000001)
        f1_score[classes] = 2*(precision[classes] * recall[classes]) / (precision[classes] + recall[classes]+.0000000001)
